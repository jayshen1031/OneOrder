...
/**
 * 简化版服务派单控制器 - 用于快速验证功能
 * 实现接派单流程的核心API，使用模拟数据
 */
@RestController
@RequestMapping("/api/service-assignment")
@CrossOrigin(origins = "*")
public class SimpleServiceAssignmentController {

    private static final Logger logger = LoggerFactory.getLogger(SimpleServiceAssignmentController.class);
    ...
    public ResponseEntity<Map<String, Object>> getAvailableOperators(
        ...
        try {
            ...
            response.put("data", Map.of(
                ...
                "availableCount", (int) operators.stream().mapToLong(op -> "AVAILABLE".equals(op.get("status")) ? 1 : 0).sum()
            ));
        ...
        try {
            ...
            for (Map<String, Object> assignment : assignments) {
                try {
                    ...
                    if (singleResult.getBody() != null && singleResult.getBody().get("code").equals(200)) {
                        results.add(Map.of(
                            "serviceCode", serviceCode,
                            "operatorId", operatorId,
                            "status", "SUCCESS",
                        ...
                        results.add(Map.of(
                            ...
                            "operatorId", operatorId,
                            "status", "FAILED",
                            "error", singleResult.getBody() != null ? singleResult.getBody().get("message") : "未知错误"
                        ));
                        failedCount++;
                    }
